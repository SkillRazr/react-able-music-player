...
(function ($) {
            ...
            thisObj.setupTracks().then(function () {
              if (thisObj.hasClosedDesc) {
                if (
                  !thisObj.$descDiv ||
                  (thisObj.$descDiv &&
                    !$.contains(thisObj.$ableDiv[0], thisObj.$descDiv[0]))
                ) {
  ...
  AblePlayer.prototype.initStenoFrame = function () {
    ...
    if (this.stenoMode && this.$stenoFrame) {
      if (
        (this.$stenoFrame[0].contentWindow, document.readyState == "complete")
      ) {
          ...
          if (thisPref === "prefDescPause" || thisPref === "prefDescVisible") {
            ...
            if (thisPref === "prefDescVoice" && this.descVoices) {
              ...
              for (j = 0; j < this.descVoices.length; j++) {
                ...
                optionLang = this.descVoices[j].lang
                  .substring(0, 2)
                  .toLowerCase();
              ...
              if (typeof options !== "undefined") {
                for (j = 0; j < options.length; j++) {
                  optionValue = options[j];
                  optionText = this.makePrefsValueReadable(
                    thisPref,
                    optionValue
                  );
                  $thisOption = $("<option>", {
            ...
            $thisField.on("change", function () {
              thisObj.announceDescriptionText(
                "sample",
                thisObj.currentSampleText
              );
            });
            $thisDiv.append($thisLabel, $thisField);
          }
    ...
    for (var i = 0; i < available.length; i++) {
      ...
      if (available[i]["label"]) {
        ...
        if (prefName === "prefDescVoice") {
          ...
          this.prefDescVoice = $("select#" + voiceSelectId)
            .find(":selected")
            .val();
          this.prefDescVoiceLang = $("select#" + voiceSelectId)
            .find(":selected")
            .attr("data-lang");
          // replace preferred voice for this lang in cookie.voices array, if one exists
          // otherwise, add it to the array
          ...
          prefName.indexOf("Desc") !== -1 &&
          prefName !== "prefDescPause" &&
          prefName !== "prefDescVisible"
        ) {
...
(function ($) {
  AblePlayer.prototype.injectPlayerCode = function () {
    ...
    this.$captionsContainer = this.$mediaContainer
      .wrap(captionsContainer)
      .parent();
    this.injectAlert();
    this.injectPlaylist();
        ...
        $menuItem.on("click mousedown", function (e) {
          ...
          if (!thisObj.windowMenuClickRegistered && !thisObj.finishingDrag) {
            ...
            thisObj.handleMenuChoice(
              which.substring(0, which.indexOf("-")),
  ...
  AblePlayer.prototype.provideFallback = function () {
    ...
    if (this.$media.attr("id")) {
      ...
      this.mediaId =
        "media" + Math.floor(Math.random() * 1000000000).toString();
    }
    ...
    if (this.$media.children().length) {
      ...
      while (i < this.$media.children().length && !this.hasFallback) {
        if (
          !(
            this.$media.children()[i].tagName === "SOURCE" ||
    ...
    this.recreatePlayer().then(function () {
      ...
      if (thisObj.showNowPlaying === true) {
        if (typeof thisObj.$nowPlayingDiv !== "undefined") {
          ...
          nowPlayingSpan.html(
            "<span>" + thisObj.tt.selectedTrack + ":</span>" + itemTitle
          );
    ...
    this.youTubePlayer = new YT.Player(containerId, {
      ...
      events: {
        onReady: function () {
          ...
          if (thisObj.playerWidth && thisObj.playerHeight) {
            thisObj.youTubePlayer.setSize(
              thisObj.playerWidth,
              thisObj.playerHeight
            );
    ...
    if (!this.youTubePlayer.getOption("captions", "tracklist")) {
      ...
      this.youTubePlayer.addEventListener("onApiChange", function (x) {
        ...
        if (typeof thisObj.prefCaptionsSize !== "undefined") {
          ...
          thisObj.youTubePlayer.setOption(
            "captions",
            "fontSize",
            thisObj.translatePrefs("size", thisObj.prefCaptionsSize, "youtube")
          );
        }
        deferred.resolve();
      });
    ...
    if (this.speechEnabled === null) {
      ...
      if (window.speechSynthesis) {
        ...
        if (context === "init") {
          ...
          // context is either 'play' or 'prefs' or 'desc'
          var greeting = new SpeechSynthesisUtterance("Hi!");
          greeting.volume = 0; // silent
          greeting.rate = 10; // fastest speed supported by the API
          thisObj.synth.speak(greeting);
...
(function ($) {
    ...
    if (captions.length === 1) {
      ...
      if (this.captionsPopup && this.captionsPopup.is(":visible")) {
        ...
        if (this.captionsPopup) {
          ...
          setTimeout(function () {
            thisObj.captionsPopup.css(
              "top",
              thisObj.$ccButton.position().top -
                thisObj.captionsPopup.outerHeight()
            );
            thisObj.captionsPopup.css(
              "left",
              thisObj.$ccButton.position().left
            );
            // Place focus on the first button (even if another button is checked)
            thisObj.captionsPopup.find("li").removeClass("able-focus");
            thisObj.captionsPopup
              .find("li")
              .first()
              .focus()
              .addClass("able-focus");
  ...
  AblePlayer.prototype.handlePrefsClick = function (pref) {
    ...
    if (this.prefsPopup.is(":visible")) {
      ...
      setTimeout(function () {
        ...
        thisObj.prefsPopup.css(
          "top",
          prefsButtonPosition.top - thisObj.prefsPopup.outerHeight()
        );
  ...
  AblePlayer.prototype.resizePlayer = function (width, height) {
    ...
    if (typeof width !== "undefined" && typeof height !== "undefined") {
      ...
      // not fullscreen, and not first time initializing player
...
(function ($) {
  ...
  AblePlayer.prototype.onMediaNewSourceLoad = function () {
    ...
    if (this.userClickedPlaylist) {
        ...
        if (this.seekStatus === "complete") {
          ...
          if (this.swapTime === this.elapsed) {
            ...
            if (this.hasDescTracks) {
              ...
              this.durationsAreCloseEnough(this.duration, this.prevDuration)
            ) {
      ...
      if (this.startTime > 0) {
        ...
        this.defaultChapter &&
        typeof this.selectedChapters !== "undefined"
      ) {
  ...
  AblePlayer.prototype.onPlayerKeyPress = function (e) {
    ...
    if (which === 27) {
      ...
      if (
        this.$transcriptArea &&
        $.contains(this.$transcriptArea[0], $thisElement[0]) &&
        !this.hidingPopup
      ) {
...
(function ($) {
  ...
  AblePlayer.prototype.getTranslationText = function () {
    ...
    if (this.lang) {
      // a data-lang attribute is included on the media element
  ...
  AblePlayer.prototype.getSampleDescriptionText = function () {
    ...
    for (i = 0; i < supportedLangs.length; i++) {
      translationFile =
        this.rootPath + "translations/" + supportedLangs[i] + ".js";
      $.getJSON(
        translationFile,
        thisLang,
        (function (thisLang) {
